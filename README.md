...
text
placed `AxiomGRulerInjector.sh`. messed up here the file is just named Inject.sh
We need to correct the filename: The user said "placed AxiomGRulerInjector.sh. messed up here the file is just named Inject.sh". So they want the README to reflect that the script file is named "Inject.sh", not "AxiomGRulerInjector.sh". The project name is still AxiomGRulerInjector, but the file is Inject.sh. I'll adjust the README accordingly: change references to "AxiomGRulerInjector.sh" to "Inject.sh", but keep the project title "AxiomGRulerInjector". Also, they want it for Bedrock, best optimized, etc. So I'll produce the corrected README with proper file name. I'll just output the full updated README.
...
- ✅ **No .exe** – just a tiny script (`Inject.sh`) that runs in Git Bash. No compiled binaries, no AV false positives.  
- ✅ **Manual map injection** – the most efficient and stealthy injection method.  
- ✅ **Absolutely zero leftover files** – creates a temporary PowerShell file **for the split second of injection**, then deletes it automatically. Nothing stays in `%TEMP%` or anywhere else.  
- ✅ **Insanely low resource usage** – near‑zero CPU and RAM during injection; the injector exits immediately after loading the DLL, leaving only Minecraft running smoothly.  
- ✅ **Works perfectly with Bedrock** – built specifically for the Windows `Minecraft.Windows.exe` (or UWP) process.  
...
1. **Launch Minecraft Bedrock** and keep it running.
2. Open **Git Bash**.
3. Navigate to the folder where you placed `Inject.sh`.
4. Run the injector with the full path to your DLL:
   ```bash
...
💡 Pro tip: You can also drag the DLL file directly into the Git Bash window after typing ./Inject.sh.

🔍 How It Works (Unmatched Efficiency)
text
Inject.sh  →  generates a tiny .ps1 (PowerShell script)  
               ↓  
            PowerShell manual‑maps the DLL into Minecraft Bedrock  
               ↓  
            DLL is fully loaded into game memory  
...
Rename Inject.sh to Inject.txt
